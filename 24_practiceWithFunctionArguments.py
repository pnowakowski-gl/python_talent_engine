def gen_mul_table(width=10, height=10, *, sep_width=3, print_header=False, print_footer=True):
    ...
    mul_table = ''.join([' ' * (sep_width-len(str(j))) + str(j) + '\n' if ind % width == width - 1 else ' ' * (sep_width-len(str(j))) + str(j) for ind, j in enumerate([i*j for i in range(1, height + 1) for j in range(1, width + 1)])])[:-1]
